...
resource "aws_cloudwatch_metric_alarm" "unhealthy-host-count" {
  ...
  alarm_name          = format("KONG01-%s-unhealthy-host-count", local.load_balancer)
  ...
  alarm_description = format("KONG01 Unhealthy host count is greater than 0 for %s", local.load_balancer)
...
resource "aws_cloudwatch_metric_alarm" "http-code-4xx-count" {
  ...
  alarm_name          = format("KONG-M-%s-http-code-4xx-count", local.load_balancer)
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
...
resource "aws_cloudwatch_metric_alarm" "http-code-5xx-count" {
  ...
  alarm_name          = format("KONG-M-%s-http-code-5xx-count", local.load_balancer)
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
